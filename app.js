var app = angular.module('oscarUI', ['ui.router', 'ui.bootstrap', 'ngResource', 'angularSpinner']);
app.run(['$state', '$rootScope', '$timeout', function ($state, $rootScope, $timeout, User, Preload, config) {
    this.init = function () {
    };
    this.init();
}]);
        ...
        Overview.getImageList($scope.filtertext, $scope.currentPage).then(function (results) {
            if (angular.isDefined(results) && results !== null) {
                if (angular.isDefined(results.photos) && results.photos.total > 0) {
                    ...
                    $scope.imageList = results.photos.photo;
